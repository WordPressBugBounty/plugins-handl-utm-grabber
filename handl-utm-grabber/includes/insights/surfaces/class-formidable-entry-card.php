...
/** Formidable entry-detail sidebar box: attribution summary (instant) + async insight. */
class Formidable_Entry_Card extends Handl_Entry_Card_Surface {
	...
	public function boot() {
		add_action( 'frm_show_entry_sidebar', array( $this, 'render' ) );
	}

	/** @param object $entry Formidable entry row (metas loaded when opened from the entries screen). */
	public function render( $entry ) {
		$posted = $this->extract_posted( $entry );
		if ( empty( $posted ) ) {
			return;
		}
		...
		self::styles();
		?>
		<div id="handl-entry-insight" class="frm_with_icons">
			<h3>HandL UTM Grabber</h3>
			<div class="inside">
	...
	private function extract_posted( $entry ) {
		if ( is_object( $entry ) && ! isset( $entry->metas ) && ! empty( $entry->id ) ) {
			$entry = \FrmEntry::getOne( (int) $entry->id, true );
		}
		return \Handl\UtmrabberFree\Submissions\Formidable_Submission_Listener::extract_from_entry( $entry );
	}
	...
	private static function styles() {
		...
		?>
		<style>
			#handl-entry-insight .inside{padding:12px;}
